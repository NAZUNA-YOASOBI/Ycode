...
因此，本项目的七个本地工具是从 DSH 基础能力中提取后独立重写的最小组合：`glob`、`grep`、`read_file`、`list_files`、`write_file`、`replace_text` 和 `run_command`。面试时应表述为“参考 DSH 的文件与命令能力，做了七工具的最小化实现”，不应表述为“DSH 官方只有七个工具”或“这是 DSH 官方的精简模式”。
...
允许且应当自行实现的流程是：文件留在本地工作目录，模型通过普通 tool calling 请求 `glob`、`grep`、`read_file`、`replace_text` 或 `run_command`，本项目的 Python 代码实际搜索、读取、修改文件或通过本地进程执行命令。模型 API 只负责生成请求，不负责执行这些编程操作。
...
- 推理模式下，模型返回的 `reasoning_content` 会作为工作过程事件显示。每个用户任务有一个大的 `Agent process` 折叠区，内部按事件实际顺序显示可单独折叠且支持 Markdown 的 Thinking、Model reply 和连续工具批次；模型回复和最终回答也使用 Markdown 渲染，工具批次仍保留 `Tools → Tool → Call/Result` 多级折叠，最终回答位于工作过程区外。
- 根据本地模型配置的上下文上限和接口返回的 `usage.prompt_tokens` 显示上下文占用百分比；压缩事件先显示压缩后的估算用量，下一次模型请求返回实际用量后覆盖；最近一次实际用量随会话保存，未知数据不自行猜测。
- 当估算请求达到上下文上限的 80% 时，用一次独立模型请求摘要较早历史，保留最近约 16% 的完整任务块和工具调用链；服务端返回上下文超限时压缩后重试一次。
- 输入严格等于 `/compact` 时直接执行一次强制历史摘要，不把命令加入模型上下文；命令会写入展示历史并显示为 `User command`，同时只保留最后一个完整任务块供模型继续使用。
- 会话历史保存到工作目录 `sessions/` 下的独立 JSON 文件，完整聊天记录（含供 Web 恢复显示的压缩摘要标记）、按实际产生顺序保存的 `display_events` 与供模型使用的压缩上下文分开记录；读取旧记录时会从消息生成展示事件，并保存标题、时间和最近一次 token 用量。
- 文件探索补充受工作目录限制的 `glob`、`grep` 工具，`read_file` 支持按 1 起始的行范围读取，减少多文件任务中的无关上下文。
- 左侧历史栏支持新建、切换和恢复会话；刷新或重新启动 Web 服务后仍可继续历史对话。
- Web 与终端共用同一套模型配置和 API key，凭据只保留在服务端环境变量中。

当前仍未实现多智能体、插件系统、并行工具、复杂上下文压缩（当前仅实现简化历史摘要）、后台任务和完整操作系统沙箱；这些能力不属于本项目的最小演示范围。
