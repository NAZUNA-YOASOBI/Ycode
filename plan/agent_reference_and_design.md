...
- 输入严格等于 `/compact` 时直接执行一次强制历史摘要，不调用主模型、不把命令写入会话，并只保留最后一个完整任务块。
- 会话历史保存到工作目录 `sessions/` 下的独立 JSON 文件，完整聊天记录（含供 Web 恢复显示的压缩摘要标记）与供模型使用的压缩上下文分开记录；读取旧记录时会从上下文摘要补回缺失的显示标记，并保存标题、时间和最近一次 token 用量。
