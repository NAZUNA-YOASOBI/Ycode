...
基于题目的真实编程任务、两分钟视频和简短面试，第一版保留文件 + Shell 两类能力作为主流程，并加入可选的 `web_search`。搜索密钥未配置时不注册该工具，文件修改和命令执行不受影响。搜索工具只通过标准库直接 HTTP 调用 DeepSeek 服务端原生搜索，不引入 MCP、Agent 框架或 DSH 依赖，也不把网页搜索误称为本地实现的搜索引擎。

搜索模型与主 Agent 模型相互独立。未指定时使用 `deepseek-v4-flash`；需要切换时通过 `DEEPSEEK_SEARCH_MODEL` 环境变量或 `--search-model` 命令行参数覆盖。第一版不请求远端模型列表，只把最终配置值写入搜索请求，以保留 DSH 的“默认值 + 可配置覆盖”行为，同时避免增加模型发现和缓存逻辑。
...
- 可选的 `web_search` 工具：配置 `DEEPSEEK_API_KEY` 后调用 DeepSeek Anthropic 兼容接口的服务端原生搜索；搜索模型默认使用 `deepseek-v4-flash`，可通过 `DEEPSEEK_SEARCH_MODEL` 或 `--search-model` 覆盖。
- 模型没有工具调用时结束。
- 最大循环次数和异常终止。
- 工作目录限制、命令超时和工具输出长度限制。
- 清楚打印模型回复、工具调用、工具结果和最终结果。
